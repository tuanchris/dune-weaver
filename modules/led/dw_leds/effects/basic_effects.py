...
# Effect registry: (name, function, uses_palette)
EFFECTS = {
    0: ("Static", mode_static, False),
    1: ("Blink", mode_blink, False),
    2: ("Breathe", mode_breath, True),
    3: ("Wipe", mode_color_wipe, False),
    4: ("Fade", mode_fade, True),
    5: ("Scan", mode_scan, True),
    6: ("Dual Scan", mode_dual_scan, True),
    7: ("Rainbow Cycle", mode_rainbow, False),
    8: ("Rainbow", mode_rainbow_cycle, False),
    9: ("Theater Chase", mode_theater_chase, True),
    10: ("Running Lights", mode_running_lights, True),
    11: ("Random Color", mode_random_color, False),
    12: ("Dynamic", mode_dynamic, False),
    13: ("Twinkle", mode_twinkle, True),
    14: ("Sparkle", mode_sparkle, True),
    15: ("Strobe", mode_strobe, False),
    16: ("Fire", mode_fire, False),
    17: ("Comet", mode_comet, True),
    18: ("Chase", mode_chase, True),
    19: ("Police", mode_police, False),
    20: ("Lightning", mode_lightning, False),
    21: ("Fireworks", mode_fireworks, False),
    22: ("Ripple", mode_ripple, True),
    23: ("Flow", mode_flow, False),
    24: ("Colorloop", mode_colorloop, False),
    25: ("Palette Flow", mode_palette_flow, True),
    26: ("Gradient", mode_gradient, False),
    27: ("Multi Strobe", mode_multi_strobe, False),
    28: ("Waves", mode_waves, True),
    29: ("BPM", mode_bpm, True),
    30: ("Juggle", mode_juggle, False),
    31: ("Meteor", mode_meteor, True),
    32: ("Pride", mode_pride, False),
    33: ("Pacifica", mode_pacifica, False),
    34: ("Plasma", mode_plasma, False),
    35: ("Dissolve", mode_dissolve, True),
    36: ("Glitter", mode_glitter, True),
    37: ("Confetti", mode_confetti, False),
    38: ("Sinelon", mode_sinelon, False),
    39: ("Candle", mode_candle, False),
    40: ("Aurora", mode_aurora, False),
    41: ("Rain", mode_rain, True),
    42: ("Halloween", mode_halloween, False),
    43: ("Noise", mode_noise, False),
    44: ("Funky Plank", mode_funky_plank, False),
}
...
def get_all_effects():
    """Get list of all effects with palette support info"""
    return [(k, v[0], v[2]) for k, v in sorted(EFFECTS.items())]
